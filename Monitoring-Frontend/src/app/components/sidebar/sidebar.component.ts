...
@Component({
  ...
  template: `
    ...
    >
      <!-- Header / Logo -->
      <div class="h-20 flex items-center px-5 border-b border-border/50 shrink-0 relative">
        <div class="flex items-center gap-3 overflow-hidden">
          <div class="w-10 h-10 bg-text rounded-xl flex items-center justify-center text-surface font-bold text-xl shadow-lg shadow-black/5 shrink-0">
            M
          </div>
          <div [class.hidden]="service.sidebarCollapsed()" class="whitespace-nowrap">
            <h1 class="text-sm font-bold text-text leading-none tracking-tight">MONITORING</h1>
            <p class="text-[10px] font-bold text-primary uppercase tracking-widest mt-0.5">Plateforme Intranet</p>
          </div>
        </div>
        ...
        <div class="space-y-2 pt-4">
          ...
          <nav class="flex flex-col gap-1">
            ...
            >
               ...
               <span *ngIf="!service.sidebarCollapsed()" class="whitespace-nowrap">Tableau de Bord</span>
            </a>
            <a 
              routerLink="/alerts" 
            ...
            >
               <div class="relative">
                 <svg class="shrink-0" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
                 <!-- Collapsed Badge -->
                 <span *ngIf="service.sidebarCollapsed() && service.alertesActivesCompte() > 0" class="absolute -top-1 -right-1 w-2 h-2 bg-rose-500 rounded-full"></span>
               </div>
               <span *ngIf="!service.sidebarCollapsed()" class="whitespace-nowrap flex items-center justify-between flex-1">
                 Alertes
                 <span *ngIf="service.alertesActivesCompte() > 0" class="text-[10px] font-black bg-rose-500 text-white px-1.5 py-0.5 rounded-md min-w-[20px] text-center">
      ...
      <!-- Footer / Action -->
      <div class="p-4 border-t border-border bg-surface/30 flex flex-col gap-3 justify-center">
        <!-- Theme Toggle -->
        <button 
        ...
        >
          ...
          <span *ngIf="!service.sidebarCollapsed()" class="text-[13px] font-bold tracking-tight whitespace-nowrap ml-2">Nouveau Client</span>
        </button>
      </div>
    </aside>
  `,
