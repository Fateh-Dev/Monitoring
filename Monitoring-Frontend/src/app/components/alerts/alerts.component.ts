...
@Component({
  ...
  template: `
    <div
      class="alerts-container p-4 md:p-10 max-w-[1200px] mx-auto space-y-8 bg-bg min-h-screen text-text"
    >
      <!-- Header -->
      <header
      ...
      >
        <div>
          <h1 class="text-3xl font-extrabold tracking-tight text-text mb-2">
            Centre d'Alertes
          </h1>
          <p class="text-text-muted text-sm max-w-md">
            Historique chronologique de tous les incidents détectés .
          </p>
        </div>

        <div
          class="flex items-center gap-2 px-4 py-2 bg-surface border border-border rounded-xl shadow-sm"
        >
          ...
          <span
            class="text-xs font-bold text-text-muted uppercase tracking-widest ml-1"
            >Incidents Actifs</span
          >
        </div>
      </header>
      ...
      <!-- Filter Tabs -->
      <div
        class="flex items-center p-1 bg-surface border border-border rounded-xl w-fit"
      >
        <button
      ...
      <div class="space-y-4">
        ...
        >
          ...
          >
            <div class="flex items-start gap-5">
              ...
              <div class="space-y-1">
                <div class="flex items-center gap-3">
                  <h3
                    class="font-bold text-lg text-text leading-none tracking-tight"
                  >
                ...
                <div class="flex items-center gap-2">
                  <p
                    class="text-[11px] font-bold text-primary uppercase tracking-widest"
                  >
                    Serveur :
                  </p>
                  <a
            ...
            <!-- Timestamps -->
            <div
              class="flex flex-col md:text-right gap-1 border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-8 min-w-[180px]"
            >
              <div class="flex items-center md:justify-end gap-2">
                <span
                  class="text-[10px] font-black text-text-muted uppercase tracking-tighter"
                  >Début :</span
                >
                <span class="text-sm font-bold text-text tabular-nums">{{
                  incident.debutLe | date: 'dd MMM, HH:mm:ss'
                }}</span>
              </div>
              <div
                class="flex items-center md:justify-end gap-2"
                *ngIf="incident.resoluLe"
              >
                <span
                  class="text-[10px] font-black text-text-muted uppercase tracking-tighter"
                ...
                <span class="text-sm font-bold text-emerald-500 tabular-nums">{{
                  incident.resoluLe | date: 'dd MMM, HH:mm:ss'
                }}</span>
              </div>
            </div>
          </div>
